...
def instant_access():
    ...
    result = soup.find("table", attrs={"data-component-type":"Table"})
...
def bonds():
    ...
    for tr in trs:
        ...
        if c == 0:
            ...
            Term_in_Months = int(product_name.split("-")[0]) * 12
            if c == 1:
                interest = AER = re.findall('(\d.*%/|\d.*%)', p[1].text) if len(re.findall('(\d.*%/|\d.*%)', p[1].text))!=0 else None
                if interest is not None:
                    if len(interest)>=2:
                        interest = AER = re.sub('[^0-9.%]','',interest[1])
                    else:
                        interest = AER = re.sub('[^0-9.%]', '', interest[0])

                Balance = tds[1].find_all('p')[-1].text.strip("(").strip(")\n")
                # Balance = tds[2].text.strip("(").strip(")\n")
                MIN = p[3].text
...
if __name__ == '__main__':
    ...
    data["Balance"] = " "+data["Balance"].apply(lambda x: re.sub('[^0-9.,-]','',str(x).replace('to','-').replace("million", "000000").replace("m", "000000")))
