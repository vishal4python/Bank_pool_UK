...
jsoup = BeautifulSoup(resp,'lxml')                                                                                  #changed line(Added lxml) remove it if some error occurs
...
df['Term in Months'] = df['Term in Months'].apply(lambda x: int(re.sub('[^0-9]','',re.search('[0-9]* year',x,re.IGNORECASE).group(0)))*12 if re.search('[0-9]* year',x,re.IGNORECASE) is not None else np.nan)
