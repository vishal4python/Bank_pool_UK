...
startTime = time.time()
path = output_path+"Aggregator_MoneySuperMarket_Data_Mortgage_"+str(today.strftime('%Y_%m_%d'))+'.csv'
today = datetime.datetime.now()
    ...
    for i in range(start_page, pages):
        try:
            ...
            for Bank in dict_from_json['results']:
                ...
                Year =test = Bank['quote']['productName']
                ...
                Balance = str(Bank['quote']['minBorrowingAmount']) #+ '-' + str(int(Bank['quote']['maxBorrowingAmount']))
                        ...
                        if Bank_Name is not  None:
                            if k in Bank_Name.lower().strip():
                                if Bank['strapline'] is None:
                                    if 'DISCOUNTED_VARIABLE'.lower() not in Bank['quote']['productName'].lower():


                                        if Bank['quote']['calcBookingFee'] != 0:
                                            if 'fixed' in test.lower():
                                                product_name = str(Year) + 'Year Fixed With Fee'
                                            else:
                                                product_name = str(Year) + 'Year Tracker With Fee'
                                            # product_name = str(Year)+ 'Year Fixed With Fee'
                                        else:
                                            # product_name = str(Year)+ 'Year Fixed'
                                            if 'fixed' in test.lower():
                                                product_name = str(Year) + 'Year Fixed'
                                            else:
                                                product_name = str(Year) + 'Year Tracker'
                                            # product_name = str(Year)+ 'Year Fixed With Fee'

                                        # if 'variable' in test.lower():
                                        #     interest_type = 'Variable'
                                        # else:
                                        #     interest_type = 'Fixed'

                                        a = [neededUkBanks[k], product_name , Balance, 'Offline', term, 'Variable', str(Interest)+'%', str(APRC)+'%',
